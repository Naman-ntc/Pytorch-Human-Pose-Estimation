...
class MPII(data.Dataset):
        ...
        def __getitem__(self, index):
                try:
                        if self.opts.Univ:
                                index = int(torch.randint(0, min(len(self.annot['scale']), self.opts.trainLimit), (1,)))
                ...
                if self.split == 'train':
                        s = s * (2 ** I.Rnd(self.opts.scale))
                        r = 0 if np.random.random() < 0.6 else I.Rnd(self.opts.rotate)
        ...
        def __len__(self):
                try:
                        if self.opts.Univ:
                                if self.split == 'train':
                                        return self.opts.valInterval*self.opts.batchSize
                                elif self.split == 'val':
                                        return self.opts.valLength*self.opts.batchSize
                                else:
